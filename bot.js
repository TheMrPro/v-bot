...
client.on('ready', () => {
   client.user.setGame(("V-help"),`Vade'Systéme`);
...
client.on("message", message => {
 if (message.content === "V-Ch ac") {
   ...
   ** بوت V أوآمر **
        للتفعيل 
V-Ch ac1  ⇏  لتفعيل عداد اعضاء السيرفر
V-Ch ac2  ⇏  لتفعيل روم والوقت
V-Ch ac3  ⇏  لتفعيل روم التاريخ
`);
...
client.on("message", message => {
 if (message.content === "V-helpvc") {
  ...
  const embed = new Discord.RichEmbed() 
      ...
      .setDescription(` 
   ** بوت V أوآمر **
V-v2 ⇏ لإنشاء روم صوتي لمدة دقيقتين
V-v4 ⇏ لإنشاء روم صوتي لمدة 4 دقائق 
V-c2 ⇏ لإنشاء روم كتابي لمدة دقيقتين
V-c4 ⇏ لإنشاء روم كتابي لمدة 4 دقائق
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
V-inv ⇏  لدعوة آلبوت لسيرفرك
...
client.on("message", message => {
 if (message.content === "V-help") {
  ...
  const embed = new Discord.RichEmbed() 
      ...
      .setDescription(` 
   ** بوت V أوآمر **
V-Build V ⇏ لأنشآء رومات كتابيه وصوتيه
V-Build Rr ⇏ لأنشآء رتب
V-role ⇏ لإعطآء عضو رتبة 
V-cv ⇏ لإنشاء روم صوتي  
V-ct ⇏ لإنشاء روم كتابي
V-dvc ⇏ لحذف الروم صوتي أم كتآبي
V-v2 ⇏ لإنشاء روم صوتي لمدة دقيقتين
V-v4 ⇏ لإنشاء روم صوتي لمدة 4 دقائق 
V-c2 ⇏ لإنشاء روم كتابي لمدة دقيقتين
V-c4 ⇏ لإنشاء روم كتابي لمدة 4 دقائق
V-cr1 ⇏ إنشآء 50 لون مرتب
V-cr2 ⇏ إنشآء 100 لون مرتب
V-cr3 ⇏ إنشاء 130 لون مرتب
V-Ch ⇏ لإنشآء روم ساعة وتاريخ وأعدآد الأعضآء
V-Ch ac ⇏ لتفعيل روم الساعة والتاريخ والاعضاء
V-delete ro ⇏ لحذف جميع الرتب بشرط رتبة البوت فوق الرتب المراد حذفها
V-delete vc ⇏ لحذف جميع الرومات الصوتية والكتابيةة
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
V-inv ⇏  لدعوة آلبوت لسيرفرك
V-uptime ⇏  لرؤيةة مدة تشغيل البوت
V-sug ⇏  لإرسآل أقترآح لمبرمج آلبوت
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
أو من خلال الرابط التالي :
https://discordapp.com/api/oauth2/authorize?client_id=423917382474399756&permissions=268528752&scope=bot
...
console.log(` V bot Added From Server -- = ${guild.name} = -- , Server Owner -- = ${guild.owner.user.username} = --`)
});


client.on("guildDelete", guild => {
console.log(`  V bot Leave From Server -- = ${guild.name} = -- , Server Owner -- = ${guild.owner.user.username} = --`)
});


client.on("guildCreate", guild => {
client.channels.get("425722377406644224").send(`
V-Bot added :white_check_mark: 
...
client.on("message", (message) => {
    if (message.content.startsWith('V-dvc')) {
  if (!message.member.hasPermission('MANAGE_CHANNELS')) return  message.reply("أنت ليس لديك برمشن  `MANAGE_CHANNELS`  ");
        let args = message.content.split(' ').slice(1);
...
client.on("message", (message) => {
    if (message.content.startsWith('V-dvfff')) {
  if (!message.member.hasPermission('MANAGE_CHANNELS')) return  message.reply("أنت ليس لديك برمشن  `MANAGE_CHANNELS`  ");
...
message.reply ("لتفعيل الرومات اكتب `V-Ch ac`");  
}
});
...
if (!muteRole) return message.reply("** قم بإنشآء الرومات اولا عن طريق الامر V-Ch **").catch(console.error);
...
message.channel.sendMessage("تم تفعيل الروم بنجاح");
   ...
   if (message.content === 'V-delete ro') {
   ...
   client.on('message', message => {
    if (message.channel.guild) {
   if (message.content === 'V-delete vc') {
	         if(!message.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return message.reply("**يحتاج البوت الى خاصية` MANAGE_CHANNELS ` **").then(msg => msg.delete(6000))
	                         if(!message.member.hasPermission('ADMINISTRATOR')) return   message.channel.send('**أنت ليس لديك برمشن** `ADMINISTRATOR`' );
...
client.on('message', message => {
  ...
  if (message.content.startsWith("V-sug")) {
...
client.on('message', message => {
	...
	if (message.content === "V-Build RRfrr") {
		      if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES")) return message.reply("**يحتاج البوت الى خاصية` MANAGE_ROLES ` **").then(msg => msg.delete(6000))
